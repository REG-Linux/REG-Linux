...
BATOCERA_ES_SYSTEM_DEPENDENCIES = host-python3 host-python-pyyaml reglinux-configgen host-gettext
BATOCERA_ES_SYSTEM_SOURCE=
BATOCERA_ES_SYSTEM_VERSION=1.03

define BATOCERA_ES_SYSTEM_BUILD_CMDS
	$(HOST_DIR)/bin/python \
		$(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/batocera-es-system.py \
		$(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/es_systems.yml        \
		$(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/es_features.yml       \
		$(@D)/es_external_translations.h \
		$(@D)/es_keys_translations.h \
                $(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera \
		$(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/locales/blacklisted-words.txt \
		$(CONFIG_DIR)/.config \
		$(@D)/es_systems.cfg \
		$(@D)/es_features.cfg \
		...
		sed -i '/^"POT-Creation-Date: /d' $(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/locales/batocera-es-system.pot

		for PO in $(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/locales/*/batocera-es-system.po; do (msgmerge -U --no-fuzzy-matching $${PO} $(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/locales/batocera-es-system.pot && printf "%s " $$(basename $$(dirname $${PO})) && LANG=C msgfmt -o /dev/null $${PO} --statistics) || exit 1; done
		...
		cp -pr $(BR2_EXTERNAL_REGLINUX_PATH)/package/batocera/emulationstation/batocera-es-system/locales $(STAGING_DIR)/usr/share/batocera-es-system
endef
