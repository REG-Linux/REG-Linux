...
class pwmled(object):
    ...
    def pwmchip_init(self, chip):
        ...
        try:
            ...
            if DEBUG:
                print(
                    "Error: PWM device is not a supported LED: {} ({})".format(
                        self.base, e
                    )
                )
            return None
    ...
    def set_color(self, rgb):
        ...
        if rgb == "PULSE":
            ...
            [r, g, b] = rgb.split(" ")
            r, g, b = (
                str(dec_to_pwm(r, self.period)),
                str(dec_to_pwm(g, self.period)),
                str(dec_to_pwm(b, self.period)),
            )
        else:
            r, g, b = rgb[0:2], rgb[2:4], rgb[4:6]
            r, g, b = (
                str(hex_to_pwm(r, self.period)),
                str(hex_to_pwm(g, self.period)),
                str(hex_to_pwm(b, self.period)),
            )
        if DEBUG:
    ...
    def get_color(self) -> str:
        l = self.led[0]
        with open(l + "/pwm0/duty_cycle", "r") as p:
            r = p.readline().strip()
        with open(l + "/pwm1/duty_cycle", "r") as p:
            g = p.readline().strip()
        with open(l + "/pwm2/duty_cycle", "r") as p:
            b = p.readline().strip()
    ...
    def set_color_dec(self, rgb):
        ...
        for n in int_list:
            ...
            if n > 255:
                n = 255
        r, g, b = (
            str(dec_to_pwm(int_list[0], self.period)),
            str(dec_to_pwm(int_list[1], self.period)),
            str(dec_to_pwm(int_list[2], self.period)),
        )
        if DEBUG:
            print(f"Set color to: {r} {g} {b}")
        for l in self.led:
