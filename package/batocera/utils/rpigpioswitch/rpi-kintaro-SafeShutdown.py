...
class SNES:
    ...
    def power_interrupt(self, channel):
        ...
        if (
            GPIO.input(self.power_pin) == GPIO.HIGH
            and GPIO.input(self.check_pin) == GPIO.LOW
        ):  # shutdown function if the powerswitch is toggled
    ...
    def reset_interrupt(self, channel):
        if GPIO.input(self.reset_pin) == GPIO.LOW:  # reset function
            ...
            while (
                GPIO.input(self.reset_pin) == GPIO.LOW
            ):  # while the button is hold the counter counts up
                self.blink(15, 0.1)
                os.system("reboot")
    ...
    def pwm_fancontrol(self, hysteresis, starttemp, temp):
        perc = 100.0 * (
            (temp - (starttemp - hysteresis)) / (starttemp - (starttemp - hysteresis))
        )
    ...
    def check_fan(self):
        self.pwm_fancontrol(
            self.fan_hysteresis, self.fan_starttemp, self.temp()
        )  # fan starts at 60 degrees and has a 5 degree hysteresis

    def attach_interrupts(self):
        if (
            GPIO.input(self.check_pin) == GPIO.LOW
        ):  # check if there is an pcb and if so attach the interrupts
            GPIO.add_event_detect(
                self.check_pin, GPIO.RISING, callback=self.pcb_interrupt
            )  # if not the interrupt gets attached
            if (
                GPIO.input(self.power_pin) == GPIO.HIGH
            ):  # when the system gets startet in the on position it gets shutdown
                os.system("shutdown -h now")
            else:
                self.led(1)
                GPIO.add_event_detect(
