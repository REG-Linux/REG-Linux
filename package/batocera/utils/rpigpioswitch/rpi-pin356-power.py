...
)  # Powerbutton - GPIO on pin 5 is the GPIO 3 in BCM mode
GPIO.setup(
    2, GPIO.IN, pull_up_down=GPIO.PUD_UP
)  # Resetbutton - GPIO on pin 3 is the GPIO 2 in BCM mode
...
def exitAllBatoceraEmulator(channel):
    ...
    os.system(
        "killall -9 retroarch PPSSPPSDL reicast.elf mupen64plus x64 scummvm dosbox vice amiberry dolphin-emu"
    )
...
GPIO.add_event_detect(
    2, GPIO.FALLING, callback=exitAllBatoceraEmulator, bouncetime=1000
)
