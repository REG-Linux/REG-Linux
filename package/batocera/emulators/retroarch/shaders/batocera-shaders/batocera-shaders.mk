...
BATOCERA_GPU_SYSTEM = none

ifeq ($(BR2_PACKAGE_BATOCERA_TARGET_X86)$(BR2_PACKAGE_BATOCERA_TARGET_X86_64_ANY),y)
	BATOCERA_GPU_SYSTEM = x86
else ifeq ($(BR2_PACKAGE_BATOCERA_TARGET_VISIONFIVE2)$(BR2_PACKAGE_BATOCERA_TARGET_BANANAPI_F3),y)
...
else ifeq ($(BR2_PACKAGE_BATOCERA_TARGET_H3)$(BR2_PACKAGE_BATOCERA_TARGET_RK3128)$(BR2_PACKAGE_BATOCERA_TARGET_RK3328)$(BR2_PACKAGE_BATOCERA_TARGET_CHA),y)
...
define BATOCERA_SHADERS_INSTALL_TARGET_CMDS
	...
	if test -e $(BATOCERA_SHADERS_DIRIN)/rendering-defaults-$(BATOCERA_GPU_SYSTEM).yml; then \
		cp $(BATOCERA_SHADERS_DIRIN)/rendering-defaults-$(BATOCERA_GPU_SYSTEM).yml $(TARGET_DIR)/usr/share/batocera/shaders/configs/rendering-defaults.yml; \
	else \
	...
	for set in $(BATOCERA_SHADERS_SETS); do \
		...
		if test -e $(BATOCERA_SHADERS_DIRIN)/$$set/rendering-defaults-$(BATOCERA_GPU_SYSTEM).yml; then \
			cp $(BATOCERA_SHADERS_DIRIN)/$$set/rendering-defaults-$(BATOCERA_GPU_SYSTEM).yml $(TARGET_DIR)/usr/share/batocera/shaders/configs/$$set/rendering-defaults.yml; \
		else \
