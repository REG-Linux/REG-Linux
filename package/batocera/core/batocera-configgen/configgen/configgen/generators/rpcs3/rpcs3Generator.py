...
class Rpcs3Generator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        if not os.path.exists(os.path.dirname(rpcs3Conf.rpcs3CurrentConfig)):
            os.makedirs(os.path.dirname(rpcs3Conf.rpcs3CurrentConfig))
        ...
        if os.path.exists(rpcs3Conf.rpcs3CurrentConfig):
            rpcsCurrentSettings.read(rpcs3Conf.rpcs3CurrentConfig)
        ...
        with open(rpcs3Conf.rpcs3CurrentConfig, "w") as configfile:
            rpcsCurrentSettings.write(configfile)

        if not os.path.exists(os.path.dirname(rpcs3Conf.rpcs3config)):
            os.makedirs(os.path.dirname(rpcs3Conf.rpcs3config))

        # Generate a default config if it doesn't exist otherwise just open the existing
        rpcs3ymlconfig = {}
        if os.path.isfile(rpcs3Conf.rpcs3config):
            with open(rpcs3Conf.rpcs3config, "r") as stream:
