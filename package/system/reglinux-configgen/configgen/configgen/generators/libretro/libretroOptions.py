...
def generateCoreSettings(
    ...
    match system.config["core"]:
        ...
        case "atari800":
            configure_atari_800(coreSettings, system)
        case "virtualjaguar":
            configure_virtual_jaguar(coreSettings, system)
        case "handy":
            configure_handy(coreSettings, system)
        case "vice_x64" | "vice_x64sc" | "vice_xscpu64":
            configure_commodore_64(coreSettings, system, guns)
        case "vice_x128":
            configure_commodore_128(coreSettings, system)
        case "vice_xplus4":
            configure_commodore_plus(coreSettings, system)
        case "vice_xvic":
        ...
        case "o2em":
            configure_o2em(coreSettings, system)
        case "mame":
            configure_mame(coreSettings, system)
        case "same_cdi":
            configure_samecdi(coreSettings, system)
        case "mame078plus":
            configure_mame2003_plus(coreSettings, system, guns)
        case "mame0139":
            configure_mame2010(coreSettings, system)
        case "vecx":
            configure_vecx(coreSettings, system)
        case "dosbox_pure":
        ...
        case "np2kai":
            configure_np2kai(coreSettings, system)
        case "mednafen_supergrafx":
            configure_mednafen_supergrafx(coreSettings, system)
        case "pcfx":
            configure_pcfx(coreSettings, system)
        case "mupen64plus-next":
            configure_mupen64plus_next(coreSettings, system, rom, wheels)
        case "parallel_n64":
            configure_parallel_n64(coreSettings, system, rom, wheels)
        case "desmume":
            configure_desmume(coreSettings, system)
        case "melonds":
            configure_melonds(coreSettings, system)
        case "melonsds":
            configure_melonsds(coreSettings, system)
        case "tgbdual":
            configure_tgbdual(coreSettings, system)
        case "gambatte":
            configure_gambatte(coreSettings, system)
        case "mgba":
            configure_mgba(coreSettings, system)
        case "vba-m":
            configure_vba_m(coreSettings, system)
        case "nestopia":
            configure_nestopia(coreSettings, system, guns)
        case "fceumm":
            configure_fceumm(coreSettings, system, guns)
        case "mesen":
            configure_mesen(coreSettings, system)
        case "pokemini":
            configure_pokemini(coreSettings, system)
        case "snes9x":
            configure_snes9x(coreSettings, system, guns)
        case "snes9x_next":
            configure_snes9x_next(coreSettings, system, guns)
        case "bsnes":
            configure_bsnes(coreSettings, system, guns)
        case "mesen-s":
            configure_mesen_s(coreSettings, system)
        case "vb":
            configure_vb(coreSettings, system)
        case "opera":
            configure_opera(coreSettings, system, rom)
        case "xrick":
            configure_xrick(coreSettings, system)
        case "scummvm":
            configure_scummvm(coreSettings, system)
        case "flycast":
            configure_flycast(coreSettings, system, guns, wheels)
        case "genesisplusgx":
            configure_genesisplusgx(coreSettings, system, guns)
        case "picodrive":
            configure_picodrive(coreSettings, system)
        case "yabasanshiro":
            configure_yabasanshiro(coreSettings, system)
        case "kronos":
            configure_kronos(coreSettings, system, guns)
        case "beetle-saturn":
            configure_beetle_saturn(coreSettings, system, guns, wheels)
        case "px68k":
            configure_px68k(coreSettings, system)
        case "81":
            configure_81(coreSettings, system)
        case "fuse":
            configure_fuse(coreSettings, system)
        case "fbneo":
            configure_fbneo(coreSettings, system, rom, guns)
        case "neocd":
            configure_neocd(coreSettings, system)
        case "ppsspp":
            configure_ppsspp(coreSettings, system)
        case "mednafen_psx":
            configure_mednafen_psx(coreSettings, system)
        case "swanstation | duckstation":
            configure_swanstation(coreSettings, system, guns)
        case "pcsx2":
            configure_pcsx2(coreSettings, system)
        case "pcsx_rearmed":
            configure_pcsx_rearmed(coreSettings, system, guns)
        case "theodore":
            configure_theodore(coreSettings, system)
        case "potator":
            configure_potator(coreSettings, system)
        case "prboom":
            configure_prboom(coreSettings, system)
        case "tyrquake":
            configure_tyrquake(coreSettings, system)
        case "mrboom":
            configure_mrboom(coreSettings, system)
        case "hatarib":
            configure_hatarib(coreSettings, system)
    ...
    if system.name == "atari800":
        ...
        if system.isOptSet("atari800_artifacting"):
            coreSettings.save(
                ...
                '"' + system.config["atari800_artifacting"] + '"',
            )
        else:
        ...
        if system.isOptSet("atari800_internalbasic"):
            coreSettings.save(
                ...
                '"' + system.config["atari800_internalbasic"] + '"',
            )
        else:
...
def configure_commodore_64(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if system.isOptSet("vice_ram_expansion_unit"):
        coreSettings.save(
            ...
            '"' + system.config["vice_ram_expansion_unit"] + '"',
        )
    else:
    ...
    if system.isOptSet("vice_keyboard_pass_through"):
        coreSettings.save(
            ...
            '"' + system.config["vice_keyboard_pass_through"] + '"',
        )
    else:
...
def configure_commodore_plus(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("vice_plus4_external_palette"):
        coreSettings.save(
            ...
            '"' + system.config["vice_plus4_external_palette"] + '"',
        )
    else:
...
def configure_commodore_vic(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("vice_vic20_external_palette"):
        coreSettings.save(
            ...
            '"' + system.config["vice_vic20_external_palette"] + '"',
        )
    else:
...
def configure_commodore_pet(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("vice_pet_external_palette"):
        coreSettings.save(
            ...
            '"' + system.config["vice_pet_external_palette"] + '"',
        )
    else:
...
def configure_commodore_puae(coreSettings: Any, system: Any) -> None:
    ...
    if (
        system.isOptSet("cpu_compatibility")
        and system.config["cpu_compatibility"] == "exact"
    ):
    ...
    if (system.name == "amiga500") or (system.name == "amiga1200"):
        ...
        if system.isOptSet("disable_joystick"):
            coreSettings.save(
                ...
                '"' + system.config["disable_joystick"] + '"',
            )
        else:
    ...
    if system.name == "amigacd32" or (system.name == "amigacdtv"):
        ...
        if system.isOptSet("puae_cd_startup_delayed_insert"):
            coreSettings.save(
                ...
                '"' + system.config["puae_cd_startup_delayed_insert"] + '"',
            )
        else:
    ...
    if system.name == "amigacd32":
        ...
        if system.isOptSet("puae_cd32pad_options"):
            coreSettings.save(
                ...
                '"' + system.config["puae_cd32pad_options"] + '"',
            )
        else:
...
def configure_o2em(coreSettings: Any, system: Any) -> None:
    ...
    # Audio Filter
    if (
        system.isOptSet("o2em_low_pass_range")
        and system.config["o2em_low_pass_range"] != "0"
    ):
...
def configure_samecdi(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("same_cdi_cpu_overclock"):
        coreSettings.save(
            ...
            '"' + system.config["same_cdi_cpu_overclock"] + '"',
        )
    else:
...
def configure_mame2003_plus(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if system.isOptSet("mame2003-plus_frameskip"):
        coreSettings.save(
            ...
            '"' + system.config["mame2003-plus_frameskip"] + '"',
        )
    else:
    ...
    if system.isOptSet("mame2003-plus_input_interface"):
        coreSettings.save(
            ...
            '"' + system.config["mame2003-plus_input_interface"] + '"',
        )
    else:
    ...
    if system.isOptSet("mame2003-plus_tate_mode"):
        coreSettings.save(
            ...
            '"' + system.config["mame2003-plus_tate_mode"] + '"',
        )
    else:
    ...
    if system.isOptSet("mame2003-plus_neogeo_bios"):
        coreSettings.save(
            ...
            '"' + system.config["mame2003-plus_neogeo_bios"] + '"',
        )
    else:
...
def configure_mame2010(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("mame_current_frame_skip"):
        coreSettings.save(
            ...
            '"' + system.config["mame_current_frame_skip"] + '"',
        )
    else:
    ...
    if system.isOptSet("mame_current_turbo_button"):
        coreSettings.save(
            ...
            '"' + system.config["mame_current_turbo_button"] + '"',
        )
    else:
    ...
    if system.isOptSet("mame_current_turbo_delay"):
        coreSettings.save(
            ...
            '"' + system.config["mame_current_turbo_delay"] + '"',
        )
    else:
...
def configure_dosbox_pure(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("pure_keyboard_layout"):
        coreSettings.save(
            ...
            '"' + system.config["pure_keyboard_layout"] + '"',
        )
    else:
    ...
    if system.isOptSet("pure_joystick_analog_deadzone"):
        coreSettings.save(
            ...
            '"' + system.config["pure_joystick_analog_deadzone"] + '"',
        )
    else:
    ...
    if system.isOptSet("pure_joystick_timed"):
        coreSettings.save(
            ...
            '"' + system.config["pure_joystick_timed"] + '"',
        )
    else:
...
def configure_bluemsx(coreSettings: Any, system: Any) -> None:
    ...
    if (
        system.isOptSet("bluemsx_nospritelimits")
        and system.config["bluemsx_nospritelimits"] == "False"
    ):
...
def configure_np2kai(coreSettings: Any, system: Any) -> None:
    ...
    if (
        system.isOptSet("np2kai_skipline")
        and system.config["np2kai_skipline"] != "Full 255 lines"
    ):
    ...
    # JAST SOUND
    if (
        system.isOptSet("np2kai_jast_snd")
        and system.config["np2kai_jast_snd"] == "True"
    ):
...
def configure_mupen64plus_next(
    ...
    if (
        system.isOptSet("mupen64plus-43screensize")
        and system.config["mupen64plus-43screensize"] != "320x240"
    ):
        coreSettings.save(
            "mupen64plus-43screensize",
            '"' + system.config["mupen64plus-43screensize"] + '"',
        )
    else:
        coreSettings.save("mupen64plus-43screensize", '"320x240"')
    # Video 16:9 Resolution
    if (
        system.isOptSet("mupen64plus-169screensize")
        and system.config["mupen64plus-169screensize"] != "640x360"
    ):
        coreSettings.save(
            "mupen64plus-169screensize",
            '"' + system.config["mupen64plus-169screensize"] + '"',
        )
    else:
    ...
    if (
        system.isOptSet("mupen64plus-aspect")
        and system.isOptSet("ratio")
        and system.isOptSet("bezel")
        and system.config["mupen64plus-aspect"] == "16:9 adjusted"
        and system.config["ratio"] == "16/9"
        and system.config["bezel"] == "none"
    ):
    ...
    # Bilinear Filtering
    if (
        system.isOptSet("mupen64plus-BilinearMode")
        and system.config["mupen64plus-BilinearMode"] == "3point"
    ):
    ...
    # Anti-aliasing (MSA)
    if (
        system.isOptSet("mupen64plus-MultiSampling")
        and system.config["mupen64plus-MultiSampling"] != "0"
    ):
        coreSettings.save(
            "mupen64plus-MultiSampling",
            '"' + system.config["mupen64plus-MultiSampling"] + '"',
        )
    else:
        coreSettings.save("mupen64plus-MultiSampling", '"0"')
    # Texture Filtering
    if (
        system.isOptSet("mupen64plus-txFilterMode")
        and system.config["mupen64plus-txFilterMode"] != "None"
    ):
        coreSettings.save(
            "mupen64plus-txFilterMode",
            '"' + system.config["mupen64plus-txFilterMode"] + '"',
        )
    else:
        coreSettings.save("mupen64plus-txFilterMode", '"None"')
    # Texture Enhancement
    if (
        system.isOptSet("mupen64plus-txEnhancementMode")
        and system.config["mupen64plus-txEnhancementMode"] != "None"
    ):
        coreSettings.save(
            "mupen64plus-txEnhancementMode",
            '"' + system.config["mupen64plus-txEnhancementMode"] + '"',
        )
    else:
    ...
    if system.isOptSet("mupen64plus-parallel-rdp-upscaling"):
        coreSettings.save(
            ...
            '"' + system.config["mupen64plus-parallel-rdp-upscaling"] + '"',
        )
    else:
    ...
    if system.isOptSet("mupen64plus-deadzone"):
        coreSettings.save(
            ...
            '"' + system.config["mupen64plus-deadzone"] + '"',
        )
    elif (
    ...
    if system.isOptSet("mupen64plus-sensitivity"):
        coreSettings.save(
            ...
            '"' + system.config["mupen64plus-sensitivity"] + '"',
        )
    else:
...
def configure_parallel_n64(
    ...
    if system.isOptSet("parallel-n64-gfxplugin"):
        coreSettings.save(
            ...
            '"' + system.config["parallel-n64-gfxplugin"] + '"',
        )
    # vulkan doesn't work with auto
    elif system.isOptSet("gfxbackend") and system.config["gfxbackend"] == "vulkan":
    ...
    if system.isOptSet("parallel-n64-screensize"):
        coreSettings.save(
            ...
            '"' + system.config["parallel-n64-screensize"] + '"',
        )
    else:
    ...
    # Increases from 4:3 to 16:9 in 3D games (bad for 2D)
    if (
        system.isOptSet("parallel-n64-aspectratiohint")
        and system.isOptSet("ratio")
        and system.isOptSet("bezel")
        and system.config["parallel-n64-aspectratiohint"] == "widescreen"
        and system.config["ratio"] == "16/9"
        and system.config["bezel"] == "none"
    ):
    ...
    if system.isOptSet("parallel-n64-filtering"):
        coreSettings.save(
            ...
            '"' + system.config["parallel-n64-filtering"] + '"',
        )
    else:
    ...
    if system.isOptSet("parallel-n64-framerate"):
        coreSettings.save(
            ...
            '"' + system.config["parallel-n64-framerate"] + '"',
        )
    else:
    ...
    if system.isOptSet("parallel-n64-deadzone"):
        coreSettings.save(
            ...
            '"' + system.config["parallel-n64-deadzone"] + '"',
        )
    elif (
        system.isOptSet("use_wheels")
        and system.getOptBoolean("use_wheels")
    ...
    if system.isOptSet("parallel-n64-sensitivity"):
        coreSettings.save(
            ...
            '"' + system.config["parallel-n64-sensitivity"] + '"',
        )
    else:
...
def configure_desmume(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("internal_resolution_desmume"):
        coreSettings.save(
            ...
            '"' + system.config["internal_resolution_desmume"] + '"',
        )
    else:
...
def configure_melonds(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("melonds_use_fw_settings"):
        coreSettings.save(
            ...
            '"' + system.config["melonds_use_fw_settings"] + '"',
        )
    else:
    ...
    if system.isOptSet("melonds_screen_layout"):
        if system.config["melonds_screen_layout"] == "Hybrid Top-Ratio2":
            ...
            coreSettings.save(
                ...
                '"' + system.config["melonds_screen_layout"] + '"',
            )
    else:
...
def configure_melonsds(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("melondsds_resolution"):
        coreSettings.save(
            ...
            '"' + system.config["melondsds_resolution"] + '"',
        )
    else:
    ...
    if system.isOptSet("melondsds_cursor_timeout"):
        coreSettings.save(
            "melonds_cursor_timeout",
            '"' + system.config["melondsds_cursor_timeout"] + '"',
        )
    else:
    ...
    if system.isOptSet("melondsds_colour"):
        coreSettings.save(
            "melonds_firmware_favorite_color",
            '"' + system.config["melondsds_colour"] + '"',
        )
    else:
    ...
    if system.isOptSet("melondsds_show_unsupported"):
        coreSettings.save(
            "melonds_show_unsupported_features",
            '"' + system.config["melondsds_show_unsupported"] + '"',
        )
    else:
        coreSettings.save("melonds_show_unsupported_features", '"disabled"')
    if system.isOptSet("melondsds_show_bios"):
        coreSettings.save(
            "melonds_show_bios_warnings",
            '"' + system.config["melondsds_show_bios"] + '"',
        )
    else:
        coreSettings.save("melonds_show_bios_warnings", '"disabled"')
    if system.isOptSet("melondsds_show_layout"):
        coreSettings.save(
            "melonds_show_current_layout",
            '"' + system.config["melondsds_show_layout"] + '"',
        )
    else:
    ...
    if system.isOptSet("melondsds_show_camera"):
        coreSettings.save(
            "melonds_show_camera_state",
            '"' + system.config["melondsds_show_camera"] + '"',
        )
    else:
...
def configure_gambatte(coreSettings: Any, system: Any) -> None:
    ...
    if system.name == "gbc":
        ...
        if system.isOptSet("gbc_color_correction"):
            coreSettings.save(
                ...
                '"' + system.config["gbc_color_correction"] + '"',
            )
        else:
    ...
    if system.name == "gb":
        ...
        if system.isOptSet("gb_colorization"):
            if (
                system.config["gb_colorization"] == "none"
            ):  # No Selection --> Classic Green
                coreSettings.save("gambatte_gb_colorization", '"internal"')
                coreSettings.save("gambatte_gb_internal_palette", '"Special 1"')
            elif (
                system.config["gb_colorization"] == "GB - Disabled"
            ):  # Disabled --> Black and White Color
                coreSettings.save("gambatte_gb_colorization", '"disabled"')
                coreSettings.save("gambatte_gb_internal_palette", '"Special 1"')
            elif (
                system.config["gb_colorization"] == "GB - SmartColor"
            ):  # Smart Coloring --> Gambatte's most colorful/appropriate color
                coreSettings.save("gambatte_gb_colorization", '"auto"')
                coreSettings.save("gambatte_gb_internal_palette", '"Special 1"')
            elif (
                system.config["gb_colorization"] == "custom"
            ):  # Custom Palettes --> Use the custom palettes in the bios/palettes folder
...
def configure_mgba(coreSettings: Any, system: Any) -> None:
    ...
    if system.name != "gba":
        ...
        if (
            system.isOptSet("color_correction")
    ...
    if system.name == "gba":
        ...
        if system.isOptSet("solar_sensor_level"):
            coreSettings.save(
                ...
                '"' + system.config["solar_sensor_level"] + '"',
            )
        else:
...
def configure_nestopia(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    # Reduce Sprite Flickering
    if (
        system.isOptSet("nestopia_nospritelimit")
        and system.config["nestopia_nospritelimit"] == "disabled"
    ):
    ...
    # Crop Overscan
    if (
        system.isOptSet("nestopia_cropoverscan")
        and system.config["nestopia_cropoverscan"] == "none"
    ):
        ...
        system.isOptSet("nestopia_cropoverscan")
        and system.config["nestopia_cropoverscan"] == "h"
    ):
        ...
        system.isOptSet("nestopia_cropoverscan")
        and system.config["nestopia_cropoverscan"] == "both"
    ):
        coreSettings.save("nestopia_overscan_h_left", '"8"')
        coreSettings.save("nestopia_overscan_h_right", '"8"')
    ...
    if system.isOptSet("nestopia_blargg_ntsc_filter"):
        coreSettings.save(
            ...
            '"' + system.config["nestopia_blargg_ntsc_filter"] + '"',
        )
    else:
    ...
    # 4 Player Adapter
    if (
        system.isOptSet("nestopia_select_adapter")
        and system.config["nestopia_select_adapter"] != "automatic"
    ):
        coreSettings.save(
            "nestopia_select_adapter",
            '"' + system.config["nestopia_select_adapter"] + '"',
        )
    else:
...
def configure_fceumm(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    # Reduce Sprite Flickering
    if (
        system.isOptSet("fceumm_nospritelimit")
        and system.config["fceumm_nospritelimit"] == "disabled"
    ):
    ...
    # Crop Overscan
    if (
        system.isOptSet("fceumm_cropoverscan")
        and system.config["fceumm_cropoverscan"] == "none"
    ):
        ...
        system.isOptSet("fceumm_cropoverscan")
        and system.config["fceumm_cropoverscan"] == "h"
    ):
        ...
        system.isOptSet("fceumm_cropoverscan")
        and system.config["fceumm_cropoverscan"] == "both"
    ):
        coreSettings.save("fceumm_overscan_h_left", '"8"')
        coreSettings.save("fceumm_overscan_h_right", '"8"')
...
def configure_mesen(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("mesen_fdsautoinsertdisk"):
        coreSettings.save(
            ...
            '"' + system.config["mesen_fdsautoinsertdisk"] + '"',
        )
    else:
...
def configure_snes9x(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if system.isOptSet("reduce_sprite_flicker"):
        coreSettings.save(
            ...
            '"' + system.config["reduce_sprite_flicker"] + '"',
        )
    else:
...
def configure_snes9x_next(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if system.isOptSet("2010_reduce_sprite_flicker"):
        coreSettings.save(
            ...
            '"' + system.config["2010_reduce_sprite_flicker"] + '"',
        )
    else:
    ...
    if system.isOptSet("2010_reduce_slowdown"):
        coreSettings.save(
            ...
            '"' + system.config["2010_reduce_slowdown"] + '"',
        )
    else:
...
def configure_mesen_s(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("mesen-s_blend_high_res"):
        coreSettings.save(
            ...
            '"' + system.config["mesen-s_blend_high_res"] + '"',
        )
    else:
    ...
    if system.isOptSet("mesen-s_cubic_interpolation"):
        coreSettings.save(
            ...
            '"' + system.config["mesen-s_cubic_interpolation"] + '"',
        )
    else:
    ...
    if system.isOptSet("mesen-s_overclock_type"):
        coreSettings.save(
            ...
            '"' + system.config["mesen-s_overclock_type"] + '"',
        )
    else:
    ...
    if system.isOptSet("mesen-s_superfx_overclock"):
        coreSettings.save(
            ...
            '"' + system.config["mesen-s_superfx_overclock"] + '"',
        )
    else:
...
def configure_opera(coreSettings: Any, system: Any, rom: str) -> None:
    ...
    if (
        system.isOptSet("game_fixes_opera")
        and system.config["game_fixes_opera"] != "disabled"
    ):
...
def configure_scummvm(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("scummvm_analog_deadzone"):
        coreSettings.save(
            ...
            '"' + system.config["scummvm_analog_deadzone"] + '"',
        )
    else:
    ...
    if system.isOptSet("scummvm_gamepad_cursor_speed"):
        coreSettings.save(
            ...
            '"' + system.config["scummvm_gamepad_cursor_speed"] + '"',
        )
    else:
...
def configure_flycast(coreSettings: Any, system: Any, guns: Any, wheels: Any) -> None:
    ...
    if system.isOptSet("reicast_synchronous_rendering"):
        coreSettings.save(
            ...
            '"' + system.config["reicast_synchronous_rendering"] + '"',
        )
    else:
    ...
    if system.isOptSet("reicast_internal_resolution"):
        coreSettings.save(
            ...
            '"' + system.config["reicast_internal_resolution"] + '"',
        )
    else:
    ...
    if system.isOptSet("reicast_anisotropic_filtering"):
        coreSettings.save(
            ...
            '"' + system.config["reicast_anisotropic_filtering"] + '"',
        )
    else:
    ...
    if system.isOptSet("reicast_frame_skipping"):
        coreSettings.save(
            ...
            '"' + system.config["reicast_frame_skipping"] + '"',
        )
    else:
    ...
    # Widescreen Cheat
    if (
        system.isOptSet("reicast_widescreen_cheats")
        and system.isOptSet("ratio")
        and system.isOptSet("bezel")
        and system.config["reicast_widescreen_cheats"] == "enabled"
        and system.config["ratio"] == "16/9"
        and system.config["bezel"] == "none"
    ):
    ...
    if (
        system.isOptSet("reicast_widescreen_hack")
        and system.isOptSet("ratio")
        and system.isOptSet("bezel")
        and system.isOptSet("reicast_widescreen_cheats")
        and system.config["reicast_widescreen_hack"] == "enabled"
        and system.config["ratio"] == "16/9"
        and system.config["bezel"] == "none"
        and system.config["reicast_widescreen_cheats"] == "disabled"
    ):
    ...
    if system.isOptSet("screen_rotation_atomiswave") and system.name == "atomiswave":
        coreSettings.save(
            ...
            '"' + system.config["screen_rotation_atomiswave"] + '"',
        )
    elif system.isOptSet("screen_rotation_naomi") and system.name == "naomi":
        coreSettings.save(
            "reicast_screen_rotation",
            '"' + system.config["screen_rotation_naomi"] + '"',
        )
    else:
...
def configure_genesisplusgx(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if system.isOptSet("gpgx_no_sprite_limit"):
        coreSettings.save(
            ...
            '"' + system.config["gpgx_no_sprite_limit"] + '"',
        )
    else:
...
def configure_picodrive(coreSettings: Any, system: Any) -> None:
    ...
    if (
        ...
        and system.config["picodrive_sprlim"] == "disabled"
    ):
    ...
    if (
        system.isOptSet("picodrive_cropoverscan")
        and system.config["picodrive_cropoverscan"] == "disabled"
    ):
...
def configure_yabasanshiro(coreSettings: Any, system: Any) -> None:
    ...
    # Multitap
    if (
        system.isOptSet("multitap_yabasanshiro")
        and system.config["multitap_yabasanshiro"] != "disabled"
    ):
    ...
    if system.isOptSet("yabasanshiro_language"):
        coreSettings.save(
            ...
            '"' + system.config["yabasanshiro_language"] + '"',
        )
    else:
...
def configure_kronos(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if (
        system.isOptSet("kronos_use_beetle_saves")
        and system.config["kronos_use_beetle_saves"] == "disabled"
    ):
    ...
    # Multitap
    if (
        system.isOptSet("kronos_multitap")
        and system.config["kronos_multitap"] != "disabled"
    ):
...
def configure_fbneo(coreSettings: Any, system: Any, rom: str, guns: Any) -> None:
    ...
    if system.isOptSet("fbneo-cpu-speed-adjust"):
        coreSettings.save(
            ...
            '"' + system.config["fbneo-cpu-speed-adjust"] + '"',
        )
    else:
...
def configure_neocd(coreSettings: Any, system: Any) -> None:
    ...
    # Per-Game saves
    if (
        system.isOptSet("neocd_per_content_saves")
        and system.config["neocd_per_content_saves"] == "False"
    ):
...
def configure_mednafen_psx(coreSettings: Any, system: Any) -> None:
    ...
    if system.isOptSet("beetle_psx_hw_cpu_freq_scale"):
        coreSettings.save(
            ...
            '"' + system.config["beetle_psx_hw_cpu_freq_scale"] + '"',
        )
    else:
        coreSettings.save(
            "beetle_psx_hw_cpu_freq_scale",
    ...
    if system.isOptSet("beetle_psx_hw_skip_bios"):
        coreSettings.save(
            ...
            '"' + system.config["beetle_psx_hw_skip_bios"] + '"',
        )
    else:
    ...
    if system.isOptSet("beetle_psx_hw_internal_resolution"):
        coreSettings.save(
            ...
            '"' + system.config["beetle_psx_hw_internal_resolution"] + '"',
        )
    else:
        coreSettings.save("beetle_psx_hw_internal_resolution", '"1x(native)"')
    # Widescreen Hack
    if (
        system.isOptSet("beetle_psx_hw_widescreen_hack")
        and system.isOptSet("ratio")
        and system.isOptSet("bezel")
        and system.config["beetle_psx_hw_widescreen_hack"] == "enabled"
        and system.config["ratio"] == "16/9"
        and system.config["bezel"] == "none"
    ):
    ...
    if system.isOptSet("beetle_psx_hw_frame_duping"):
        coreSettings.save(
            "beetle_psx_hw_frame_duping",
            '"' + system.config["beetle_psx_hw_frame_duping"] + '"',
        )
    else:
    ...
    if system.isOptSet("beetle_psx_hw_cpu_dynarec"):
        coreSettings.save(
            ...
            '"' + system.config["beetle_psx_hw_cpu_dynarec"] + '"',
        )
    else:
    ...
    if system.isOptSet("beetle_psx_hw_dynarec_invalidate"):
        coreSettings.save(
            ...
            '"' + system.config["beetle_psx_hw_dynarec_invalidate"] + '"',
        )
    else:
    ...
    # Multitap
    if (
        system.isOptSet("multitap_mednafen")
        and system.config["multitap_mednafen"] != "disabled"
    ):
...
def configure_swanstation(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    elif system.isOptSet("gfxbackend"):
        if system.config["gfxbackend"] == "vulkan":
            coreSettings.save("swanstation_GPU_Renderer", '"Vulkan"')
        elif (
            system.config["gfxbackend"] == "opengl"
            or system.config["gfxbackend"] == "glcore"
        ):
    ...
    if system.isOptSet("swanstation_PatchFastBoot"):
        coreSettings.save(
            ...
            '"' + system.config["swanstation_PatchFastBoot"] + '"',
        )
    else:
    ...
    if system.isOptSet("swanstation_resolution_scale"):
        coreSettings.save(
            ...
            '"' + system.config["swanstation_resolution_scale"] + '"',
        )
    else:
    ...
    if system.isOptSet("swanstation_antialiasing"):
        coreSettings.save(
            ...
            '"' + system.config["swanstation_antialiasing"] + '"',
        )
    else:
    ...
    if system.isOptSet("swanstation_texture_filtering"):
        coreSettings.save(
            ...
            '"' + system.config["swanstation_texture_filtering"] + '"',
        )
    else:
        coreSettings.save("swanstation_GPU_TextureFilter", '"Nearest"')
    # Widescreen Hack
    if (
        system.isOptSet("swanstation_widescreen_hack")
        and system.isOptSet("ratio")
        and system.isOptSet("bezel")
        and system.config["swanstation_widescreen_hack"] == "true"
        and system.config["ratio"] == "16/9"
        and system.config["bezel"] == "none"
    ):
    ...
    if system.isOptSet("swanstation_CropMode"):
        coreSettings.save(
            ...
            '"' + system.config["swanstation_CropMode"] + '"',
        )
    else:
...
def configure_pcsx_rearmed(coreSettings: Any, system: Any, guns: Any) -> None:
    ...
    if system.isOptSet("show_bios_bootlogo"):
        coreSettings.save(
            ...
            '"' + system.config["show_bios_bootlogo"] + '"',
        )
    else:
    ...
    if (
        system.isOptSet("game_fixes_pcsx")
        and system.config["game_fixes_pcsx"] != "disabled"
    ):
    ...
    for player in [{"id": 1, "color": "red"}, {"id": 2, "color": "blue"}]:
        if system.isOptSet("pcsx_rearmed_crosshair" + str(player["id"])):
            coreSettings.save(
                "pcsx_rearmed_crosshair" + str(player["id"]),
...
def configure_tyrquake(coreSettings: Any, system: Any) -> None:
    ...
    # Frame rate
    if (
        system.isOptSet("tyrquake_framerate")
