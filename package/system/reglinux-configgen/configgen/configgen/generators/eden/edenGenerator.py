...
class EdenGenerator(Generator):
    ...
    def generate(
        ...
        EdenGenerator.writeEdenConfig(
            CONF + "/eden/qt-config.ini", system, playersControllers
        )
    ...
    def writeEdenConfig(configFile, system, playersControllers):
        ...
        if system.isOptSet("eden_single_window"):
            edenConfig.set(
                "UI", "singleWindowMode", system.config["eden_single_window"]
            )
        else:
        ...
        if system.isOptSet("eden_nvdec_emu"):
            edenConfig.set(
                "Renderer", "nvdec_emulation", system.config["eden_nvdec_emu"]
            )
        else:
        ...
        if system.isOptSet("eden_anisotropy"):
            edenConfig.set(
                "Renderer", "max_anisotropy", system.config["eden_anisotropy"]
            )
        else:
        ...
        if system.isOptSet("eden_scale_filter"):
            edenConfig.set(
                "Renderer", "scaling_filter", system.config["eden_scale_filter"]
            )
        else:
        ...
        if system.isOptSet("eden_language"):
            ...
            edenConfig.set(
                "System", "language_index", EdenGenerator.getLangFromEnvironment()
            )
        ...
        if system.isOptSet("eden_region"):
            ...
            edenConfig.set(
                "System", "region_index", EdenGenerator.getRegionFromEnvironment()
            )
        ...
        if system.isOptSet("eden_dock_mode"):
            edenConfig.set(
                "Controls", "use_docked_mode", system.config["eden_dock_mode"]
            )
        else:
        ...
        if system.isOptSet("eden_timezone"):
            edenConfig.set(
                "Controls", "time_zone_index", system.config["eden_timezone"]
            )
        else:
        ...
        for playercontroller, pad in sorted(playersControllers.items()):
            ...
            for x in edenButtonsMapping:
                edenConfig.set(
                    ...
                    '"{}"'.format(
                        EdenGenerator.setButton(
                            edenButtonsMapping[x], pad.guid, pad.inputs, nplayer - 1
                        )
                    ),
                )
            for x in edenAxisMapping:
                edenConfig.set(
                    ...
                    "player_" + str(nplayer - 1) + "_" + x,
                    '"{}"'.format(
                        EdenGenerator.setAxis(
                            edenAxisMapping[x], pad.guid, pad.inputs, nplayer - 1
                        )
                    ),
                )
            edenConfig.set(
            ...
            edenConfig.set(
                "Controls", "player_" + str(nplayer - 1) + "_connected", "true"
            )
            edenConfig.set(
                "Controls",
        ...
        if key in padInputs:
            ...
            if input.type == "button":
                ...
                return ("engine:sdl,hat:{},direction:{},guid:{},port:{}").format(
                    input.id,
                    EdenGenerator.hatdirectionvalue(input.value),
                    padGuid,
                    port,
                )
            elif input.type == "axis":
                return (
                    "engine:sdl,threshold:{},axis:{},guid:{},port:{},invert:{}"
                ).format(0.5, input.id, padGuid, port, "+")
        return ""
