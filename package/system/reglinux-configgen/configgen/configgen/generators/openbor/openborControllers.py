...
def setupControllers(config, playersControllers, joy_max_inputs, new_axis_vals):
    ...
    for playercontroller, pad in sorted(playersControllers.items()):
        config.save(
            "keys." + str(idx) + ".0",
            JoystickValue("up", pad, joy_max_inputs, new_axis_vals),
        )  # MOVEUP
        config.save(
            "keys." + str(idx) + ".1",
            JoystickValue("down", pad, joy_max_inputs, new_axis_vals),
        )  # MOVEDOWN
        config.save(
            "keys." + str(idx) + ".2",
            JoystickValue("left", pad, joy_max_inputs, new_axis_vals),
        )  # MOVELEFT
        config.save(
            "keys." + str(idx) + ".3",
            JoystickValue("right", pad, joy_max_inputs, new_axis_vals),
        )  # MOVERIGHT
        config.save(
            "keys." + str(idx) + ".4",
            JoystickValue("b", pad, joy_max_inputs, new_axis_vals),
        )  # ATTACK
        config.save(
            "keys." + str(idx) + ".5",
            JoystickValue("x", pad, joy_max_inputs, new_axis_vals),
        )  # ATTACK2
        config.save(
            "keys." + str(idx) + ".6",
            JoystickValue("pageup", pad, joy_max_inputs, new_axis_vals),
        )  # ATTACK3
        config.save(
            "keys." + str(idx) + ".7",
            JoystickValue("pagedown", pad, joy_max_inputs, new_axis_vals),
        )  # ATTACK4
        config.save(
            "keys." + str(idx) + ".8",
            JoystickValue("a", pad, joy_max_inputs, new_axis_vals),
        )  # JUMP
        config.save(
            "keys." + str(idx) + ".9",
            JoystickValue("y", pad, joy_max_inputs, new_axis_vals),
        )  # SPECIAL
        config.save(
            "keys." + str(idx) + ".10",
            JoystickValue("start", pad, joy_max_inputs, new_axis_vals),
        )  # START
        config.save(
            "keys." + str(idx) + ".11",
            JoystickValue("l2", pad, joy_max_inputs, new_axis_vals),
        )  # SCREENSHOT
        ...
        if idx == 0:
            config.save(
                "keys." + str(idx) + ".12",
                JoystickValue("hotkey", pad, joy_max_inputs, new_axis_vals),
            )  # ESC
        else:
        ...
        config.save(
            "keys." + str(idx) + ".13",
            JoystickValue("joystick1up", pad, joy_max_inputs, new_axis_vals),
        )  # axis up
        config.save(
            "keys." + str(idx) + ".14",
            JoystickValue("joystick1up", pad, joy_max_inputs, new_axis_vals, True),
        )  # axis down
        config.save(
            "keys." + str(idx) + ".15",
            JoystickValue("joystick1left", pad, joy_max_inputs, new_axis_vals),
        )  # axis left
        config.save(
            "keys." + str(idx) + ".16",
            JoystickValue("joystick1left", pad, joy_max_inputs, new_axis_vals, True),
        )  # axis right
