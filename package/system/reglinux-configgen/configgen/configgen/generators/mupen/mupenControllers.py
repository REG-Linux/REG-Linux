                    ...
                    if input.attributes:
                        if input.attributes["name"]:
                            if input.attributes["value"]:
                                map[input.attributes["name"].value] = input.attributes[
                                    "value"
                                ].value
    return map
...
def defineControllerKeys(nplayer, controller, system, isWheel):
    ...
    if (
        controller.guid in valid_n64_controller_guids
        and controller.configName in valid_n64_controller_names
    ) or (
        f"mupen64-controller{nplayer}" in system.config
        and system.config[f"mupen64-controller{nplayer}"] != "retropad"
    ):
    ...
    config["AnalogPeak"] = getJoystickPeak(
        mupenmapping["AnalogPeak"], f"mupen64-sensitivity{nplayer}", system
    )
    ...
    if isWheel:
        ...
        config["AnalogDeadzone"] = getJoystickDeadzone(
            mupenmapping["AnalogPeak"], f"mupen64-deadzone{nplayer}", system
        )
    ...
    for inputIdx in controller.inputs:
        ...
        if input.name in mupenmapping and mupenmapping[input.name] != "":
            value = setControllerLine(
                mupenmapping, input, mupenmapping[input.name], controller.inputs
            )
...
def setControllerLine(mupenmapping, input, mupenSettingName, allinputs):
    ...
    if inputType == "button":
        ...
        if mupenSettingName in ["X Axis", "Y Axis"]:  # special case for these 2 axis...
            if (
                input.value == "1" or input.value == "8"
            ):  # only for the lower value to avoid duplicate
