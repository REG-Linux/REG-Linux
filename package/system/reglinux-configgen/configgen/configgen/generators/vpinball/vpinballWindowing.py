...
def configureWindowing(
    ...
    # Playfield
    if not (
        system.isOptSet("vpinball_playfield")
        and system.config["vpinball_playfield"] == "manual"
    ):
    ...
    elif screens[playFieldScreen]["width"] < screens[playFieldScreen]["height"]:
...
def getFlexDmdConfiguration(system: Any, screens: Any, hasDmd: bool) -> str:
    val = ""
...
def getPinmameConfiguration(system: Any, screens: Any) -> str:
...
def getB2sConfiguration(system: Any, screens: Any) -> str:
    val = ""
...
def getB2sdmdConfiguration(system: Any, screens: Any, hasDmd: bool) -> bool:
    if system.isOptSet("vpinball_b2sdmd") and not system.getOptBoolean(
        "vpinball_b2sdmd",
    ):  # switchon
        return False
    return not hasDmd
...
def configurePlayfield(
    ...
    if screens[playFieldScreen]["width"] < screens[playFieldScreen]["height"]:
        screens[playFieldScreen]["width"], screens[playFieldScreen]["height"] = (
            screens[playFieldScreen]["height"],
            screens[playFieldScreen]["width"],
        )
    ...
    if pinmame_config == "screen2":
        if b2s_config == "screen2":  # share with b2s screen
            vpinballSettings.set(
                ...
                WindowName + "X",
                str(
                    screens[backglassScreen]["x"]
                    + (screens[backglassScreen]["width"] - dmdsize[0]) // 2,
                ),
            )
            vpinballSettings.set(
                "Standalone",
        ...
        vpinballSettings.set(
            "Standalone",
            WindowName + "Width",
            ConvertToPixel(gameResolution["width"], width),
        )
        vpinballSettings.set(
            "Standalone",
            WindowName + "Height",
            ConvertToPixel(gameResolution["height"], height),
        )


def getDMDWindowSize(system: Any, gameResolution: Any) -> list[int]:
    ...
    if b2s_config == "screen2":
        if (
            ...
            vpinballSettings.set(
                "Standalone",
                WindowName + "Width",
                str(screens[backglassScreen]["width"]),
            )
            vpinballSettings.set(
                "Standalone",
                WindowName + "Height",
                str(screens[backglassScreen]["height"] - dmdsize[1]),
            )
        else:
            vpinballSettings.set(
                "Standalone",
            ...
            vpinballSettings.set(
                "Standalone",
                WindowName + "Height",
                str(screens[backglassScreen]["height"]),
            )
    else:
...
def RelativeHeightCalculate(Rscreen: float, Relement: float, RelativeWidth: int) -> int:
