...
def get_AltMapping(system: Any, nplayer: int, anyMapping: Any) -> Any:
    ...
    if (
        system.isOptSet(f"dolphin_port_{nplayer}_type")
        and system.config[f"dolphin_port_{nplayer}_type"] == "6b"
    ):
        mapping["a"] = "Buttons/B"
        mapping["b"] = "Buttons/A"
    ...
    if (
        not system.isOptSet(f"dolphin_port_{nplayer}_type")
        or system.config.get(f"dolphin_port_{nplayer}_type") == "6a"
    ):
    ...
    for x in pad.inputs:
        ...
        if system.isOptSet("dsmotion") and system.getOptBoolean("dsmotion"):
            f.write(
                ...
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Gyro X-`\n"
            )
            f.write(
                "IMUGyroscope/Pitch Down = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Gyro X+`\n"
            ...
            f.write(
                "IMUGyroscope/Roll Left = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Gyro Z-`\n"
            )
            f.write(
                "IMUGyroscope/Roll Right = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Gyro Z+`\n"
            ...
            f.write(
                "IMUGyroscope/Yaw Left = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Gyro Y-`\n"
            )
            f.write(
                "IMUGyroscope/Yaw Right = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Gyro Y+`\n"
            ...
            f.write(
                "IMUAccelerometer/Left = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Accel X-`\n"
            )
            f.write(
                "IMUAccelerometer/Right = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Accel X+`\n"
            ...
            f.write(
                "IMUAccelerometer/Forward = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Accel Z-`\n"
            )
            f.write(
                "IMUAccelerometer/Backward = `evdev/"
                + str(nsamepad).strip()
                + "/"
                + pad.name.strip()
                + " Motion Sensors:Accel Z+`\n"
        ...
        if system.isOptSet(f"deadzone_{nplayer}"):
            f.write(
                f"Main Stick/Dead Zone = {system.config['deadzone_' + str(nplayer)]}\n"
            )
            f.write(
                f"C-Stick/Dead Zone = {system.config['deadzone_' + str(nplayer)]}\n"
            )
        else:
        ...
        # JS gate size
        if (
            system.isOptSet(f"jsgate_size_{nplayer}")
            and system.config[f"jsgate_size_{nplayer}"] != "normal"
        ):
...
def write_key(
    ...
    if input_type == "button":
        ...
        prefix = (
            "Full " if keyname in {"Triggers/L-Analog", "Triggers/R-Analog"} else ""
        )
