...
def generateControllerConfig_any(system, playersControllers, wheels, filename, anyDefKey, anyMapping, anyReverseAxes, anyReplacements, extraOptions = {}):
    ...
    for playercontroller, pad in sorted(playersControllers.items()):
        ...
        f.write("[" + anyDefKey + str(nplayer) + "]" + "\n")
        f.write("Device = evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + "\n")
...
def generateControllerConfig_any_auto(f, pad, anyMapping, anyReverseAxes, anyReplacements, extraOptions, system, nplayer, nsamepad):
    ...
    for x in pad.inputs:
        ...
        if system.isOptSet("dsmotion") and system.getOptBoolean("dsmotion") == True:
            f.write("IMUGyroscope/Pitch Up = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Gyro X-`\n")
            f.write("IMUGyroscope/Pitch Down = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Gyro X+`\n")
            f.write("IMUGyroscope/Roll Left = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Gyro Z-`\n")
            f.write("IMUGyroscope/Roll Right = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Gyro Z+`\n")
            f.write("IMUGyroscope/Yaw Left = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Gyro Y-`\n")
            f.write("IMUGyroscope/Yaw Right = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Gyro Y+`\n")
            f.write("IMUIR/Recenter = `Button 10`\n")
            f.write("IMUAccelerometer/Left = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Accel X-`\n")
            f.write("IMUAccelerometer/Right = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Accel X+`\n")
            f.write("IMUAccelerometer/Forward = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Accel Z-`\n")
            f.write("IMUAccelerometer/Backward = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Accel Z+`\n")
            f.write("IMUAccelerometer/Up = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Accel Y-`\n")
            f.write("IMUAccelerometer/Down = `evdev/" + str(nsamepad).strip() + "/" + pad.name.strip() + " Motion Sensors:Accel Y+`\n")
