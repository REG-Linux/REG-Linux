...
class RazeGenerator(Generator):
    ...
    def generate(
        ...
        if not Path(RAZE_CONFIG_FILE).exists():
            with Path(RAZE_CONFIG_FILE).open("w") as config:
        ...
        config_backup = []
        if Path(RAZE_CONFIG_FILE).exists():
            with Path(RAZE_CONFIG_FILE).open() as original_file:
                config_backup = original_file.readlines()

        with Path(RAZE_CONFIG_FILE).open("w") as config_file:
        ...
        with Path(RAZE_SCRIPT_FILE).open("w") as script:
            script.write(
