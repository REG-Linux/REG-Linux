...
def setDuckstationControllers(duckstatonConfig, system, metadata, guns, playersControllers):
    ## [ControllerPorts]
    ...
    for controller, pad in sorted(playersControllers.items()):
        if nplayer <= 8:
            ...
            if system.isOptSet("duckstation_" + ctrl_num):
                duckstatonConfig.set(pad_num, "Type", system.config["duckstation_" + ctrl_num])
            else:
            ...
            if system.isOptSet("duckstation_digitalmode"):
                duckstatonConfig.set(pad_num, "AnalogDPadInDigitalMode", system.config["duckstation_digitalmode"])
            ...
            if system.isOptSet("use_guns") and system.getOptBoolean("use_guns") and len(guns) > 0:
                ...
                if pedalkey is None:
                    ...
                    duckstatonConfig.set(pad_num, "A", gun_num+"/RightButton & Keyboard/"+pedalkey.upper())
                ###
                ...
                if system.isOptSet("duckstation_" + ctrl_num) and system.config["duckstation_" + ctrl_num] == "GunCon":
                    ...
                    duckstatonConfig.set(pad_num, "ShootOffscreen", sdl_num+"/+LeftTrigger")
                    duckstatonConfig.set(pad_num, "A", sdl_num+"/A")
                    duckstatonConfig.set(pad_num, "B", sdl_num+"/B")
            # Guns crosshair
            if system.isOptSet("duckstation_crosshair"):
                duckstatonConfig.set(pad_num, "CrosshairScale", system.config["duckstation_crosshair"])
            else:
