...
FLYCAST_VMU_BLANK_PATH = '/usr/share/reglinux/configgen/data/dreamcast/vmu_save_blank.bin'
...
def setFlycastConfig(flycastConfig, system, gameResolution):
    ...
    if system.isOptSet("flycast_render_resolution"):
        flycastConfig.set("config", "rend.Resolution", str(system.config["flycast_render_resolution"]))
    else:
        flycastConfig.set("config", "rend.Resolution", "480")

    # wide screen mode - default off
    if system.isOptSet("flycast_ratio"):
        flycastConfig.set("config", "rend.WideScreen", str(system.config["flycast_ratio"]))
    else:
        flycastConfig.set("config", "rend.WideScreen", "no")

    # rotate option - default off
    if system.isOptSet("flycast_rotate"):
        flycastConfig.set("config", "rend.Rotate90", str(system.config["flycast_rotate"]))
    else:
    ...
    if system.isOptSet("flycast_anisotropic"):
        flycastConfig.set("config", "rend.AnisotropicFiltering", str(system.config["flycast_anisotropic"]))
    else:
    ...
    if system.isOptSet("flycast_language"):
        flycastConfig.set("config", "Dreamcast.Language", str(system.config["flycast_language"]))
    else:
        flycastConfig.set("config", "Dreamcast.Language", "1")

    # region
    if system.isOptSet("flycast_region"):
        flycastConfig.set("config", "Dreamcast.Region", str(system.config["flycast_language"]))
    else:
        flycastConfig.set("config", "Dreamcast.Region", "1")

    # save / load states
    if system.isOptSet("flycast_loadstate"):
        flycastConfig.set("config", "Dreamcast.AutoLoadState", str(system.config["flycast_loadstate"]))
    else:
        flycastConfig.set("config", "Dreamcast.AutoLoadState", "no")
    if system.isOptSet("flycast_savestate"):
        flycastConfig.set("config", "Dreamcast.AutoSaveState", str(system.config["flycast_savestate"]))
    else:
        flycastConfig.set("config", "Dreamcast.AutoSaveState", "no")

    # windows CE
    if system.isOptSet("flycast_winCE"):
        flycastConfig.set("config", "Dreamcast.ForceWindowsCE", str(system.config["flycast_winCE"]))
    else:
        flycastConfig.set("config", "Dreamcast.ForceWindowsCE", "no")

    # DSP
    if system.isOptSet("flycast_DSP"):
         flycastConfig.set("config", "aica.DSPEnabled", str(system.config["flycast_DSP"]))
    else:
    ...
    if system.isOptSet("flycast_lightgun1_crosshair"):
        flycastConfig.set("config", "rend.CrossHairColor1", str(system.config["flycast_lightgun1_crosshair"]))
    else:
        flycastConfig.set("config", "rend.CrossHairColor1", "0")
    if system.isOptSet("flycast_lightgun2_crosshair"):
        flycastConfig.set("config", "rend.CrossHairColor2", str(system.config["flycast_lightgun2_crosshair"]))
    else:
        flycastConfig.set("config", "rend.CrossHairColor2", "0")
    if system.isOptSet("flycast_lightgun3_crosshair"):
        flycastConfig.set("config", "rend.CrossHairColor3", str(system.config["flycast_lightgun3_crosshair"]))
    else:
        flycastConfig.set("config", "rend.CrossHairColor3", "0")
    if system.isOptSet("flycast_lightgun4_crosshair"):
        flycastConfig.set("config", "rend.CrossHairColor4", str(system.config["flycast_lightgun4_crosshair"]))
    else:
