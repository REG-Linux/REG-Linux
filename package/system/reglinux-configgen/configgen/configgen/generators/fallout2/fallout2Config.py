...
def setFalloutConfig(falloutConfig, system):
    ...
    if system.isOptSet("fout2_game_difficulty"):
        falloutConfig.set("preferences", "game_difficulty", system.config["fout2_game_difficulty"])
    else:
        falloutConfig.set("preferences", "game_difficulty", "1")

    if system.isOptSet("fout2_combat_difficulty"):
        falloutConfig.set("preferences", "combat_difficulty", system.config["fout2_combat_difficulty"])
    else:
        falloutConfig.set("preferences", "combat_difficulty", "1")

    if system.isOptSet("fout2_violence_level"):
        falloutConfig.set("preferences", "violence_level", system.config["fout2_violence_level"])
    else:
        falloutConfig.set("preferences", "violence_level", "2")

    if system.isOptSet("fout2_subtitles"):
        falloutConfig.set("preferences", "subtitles", system.config["fout2_subtitles"])
    else:
        falloutConfig.set("preferences", "subtitles", "0")

    if system.isOptSet("fout2_language"):
        falloutConfig.set("system", "language", system.config["fout2_language"])
    else:
