...
from configgen.config.paths import BIOS, CONF, SAVES, SCREENSHOTS

MUPEN64PLUS_CONFIG_DIR = str(CONF / "mupen64plus")
MUPEN64PLUS_CONFIG_PATH = str(CONF / "mupen64plus" / "mupen64plus.cfg")
MUPEN64PLUS_SAVES_DIR = str(SAVES / "n64")
MUPEN64PLUS_BIN_PATH = Path("/usr/bin/mupen64plus")


def setMupen64plusConfig(
    iniConfig: Any, system: Any, gameResolution: dict[str, int]
) -> None:
    ...
    iniConfig.set(
        ...
        "Version",
        "1.01",
    )  # Version is important for the .ini creation otherwise, mupen64plus removes the section
    iniConfig.set("Core", "ScreenshotPath", str(SCREENSHOTS))
    iniConfig.set("Core", "SaveStatePath", MUPEN64PLUS_SAVES_DIR)
    iniConfig.set("Core", "SaveSRAMPath", MUPEN64PLUS_SAVES_DIR)
    iniConfig.set("Core", "SharedDataPath", MUPEN64PLUS_CONFIG_DIR)
    iniConfig.set(
        "Core",
        "SaveFilenameFormat",
        "1000",
    ...
    if (
        system.isOptSet("mupen64plus_DisableExtraMem")
        and system.config["mupen64plus_DisableExtraMem"] == "True"
    ):
    ...
    if (
        system.isOptSet("mupen64plus_AudioSync")
        and system.config["mupen64plus_AudioSync"] == "True"
    ):
    ...
    if (
        system.isOptSet("mupen64plus_ratio")
        and system.config["mupen64plus_ratio"] == "16/9"
    ) or (
        not system.isOptSet("mupen64plus_ratio")
        ...
        system.isOptSet("mupen64plus_ratio")
        and system.config["mupen64plus_ratio"] == "4/3"
    ) or (
        not system.isOptSet("mupen64plus_ratio")
        and system.isOptSet("ratio")
        and system.config["ratio"] == "4/3"
    ):
        # 4/3
    ...
    if (
        system.isOptSet("mupen64plus_Mipmapping")
        and system.config["mupen64plus_Mipmapping"] != 0
    ):
    ...
    if (
        system.isOptSet("mupen64plus_Anisotropic")
        and system.config["mupen64plus_Anisotropic"] != 0
    ):
        iniConfig.set(
            "Video-Rice",
            "AnisotropicFiltering",
            system.config["mupen64plus_Anisotropic"],
        )
        iniConfig.set(
            "Video-Glide64mk2",
            "wrpAnisotropic",
    ...
    # Anti-aliasing MSAA
    if (
        system.isOptSet("mupen64plus_AntiAliasing")
        and system.config["mupen64plus_AntiAliasing"] != 0
    ):
        iniConfig.set(
            "Video-Rice",
            "MultiSampling",
            system.config["mupen64plus_AntiAliasing"],
        )
        iniConfig.set(
            "Video-Glide64mk2",
            "wrpAntiAliasing",
    ...
    # Hires textures
    if (
        system.isOptSet("mupen64plus_LoadHiResTextures")
        and system.config["mupen64plus_LoadHiResTextures"] == "True"
    ):
    ...
    if (
        system.isOptSet("mupen64plus_TextureEnhancement")
        and system.config["mupen64plus_TextureEnhancement"] != 0
    ):
        iniConfig.set(
            "Video-Rice",
            "TextureEnhancement",
            system.config["mupen64plus_TextureEnhancement"],
        )
    else:
        iniConfig.set(
            "Video-Rice",
    ...
    if (
        system.isOptSet("mupen64plus_frameskip")
        and system.config["mupen64plus_frameskip"] != 0
    ):
        if system.config["mupen64plus_frameskip"] == "=automatic":
            ...
            iniConfig.set(
                "Video-Glide64mk2",
                "maxframeskip",
                system.config["mupen64plus_frameskip"],
            )
    else:
    ...
    if (
        system.isOptSet("mupen64plus_fb_read_always")
        and system.config["mupen64plus_fb_read_always"] != "-1"
    ):
        iniConfig.set(
            "Video-Glide64mk2",
            "fb_read_always",
            system.config["mupen64plus_fb_read_always"],
        )
    else:
