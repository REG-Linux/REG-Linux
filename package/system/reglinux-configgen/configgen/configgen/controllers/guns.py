    ...
    for eventid in sorted(mouses):
        eslog.info(
            "found mouse {} at {} with id_mouse={}".format(
                nmouse, mouses[eventid].device_node, nmouse
            )
        )
        if (
            "ID_INPUT_GUN" not in mouses[eventid].properties
            or mouses[eventid].properties["ID_INPUT_GUN"] != "1"
        ):
            nmouse = nmouse + 1
            continue
        ...
        need_cross = (
            "ID_INPUT_GUN_NEED_CROSS" in mouses[eventid].properties
            and mouses[eventid].properties["ID_INPUT_GUN_NEED_CROSS"] == "1"
        )
        need_borders = (
            "ID_INPUT_GUN_NEED_BORDERS" in mouses[eventid].properties
            and mouses[eventid].properties["ID_INPUT_GUN_NEED_BORDERS"] == "1"
        )
        guns[ngun] = {
            "node": mouses[eventid].device_node,
            "id_mouse": nmouse,
        ...
        eslog.info(
            "found gun {} at {} with id_mouse={} ({})".format(
                ngun, mouses[eventid].device_node, nmouse, guns[ngun]["name"]
            )
        )
        nmouse = nmouse + 1
        ngun = ngun + 1
