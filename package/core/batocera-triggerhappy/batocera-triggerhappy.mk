...
define BATOCERA_TRIGGERHAPPY_INSTALL_CONFIG
	...
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-triggerhappy/conf/multimedia_keys.conf          $(TARGET_DIR)/etc/triggerhappy/triggers.d
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-triggerhappy/conf/multimedia_keys_disabled.conf $(TARGET_DIR)/etc/triggerhappy/triggers.d
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-triggerhappy/triggerhappy.service  $(TARGET_DIR)/etc/init.d/S50triggerhappy
endef

define BATOCERA_TRIGGERHAPPY_INSTALL_RK3326_CONFIG
    cp -v $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-triggerhappy/conf/rk3326/*.conf      $(TARGET_DIR)/etc/triggerhappy/triggers.d/
endef
...
define BATOCERA_TRIGGERHAPPY_INSTALL_RK3128_CONFIG
	cp -v $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-triggerhappy/conf/rk3128/*.conf      $(TARGET_DIR)/etc/triggerhappy/triggers.d/
endef

define BATOCERA_TRIGGERHAPPY_INSTALL_X86_64_CONFIG
	cp -v $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-triggerhappy/conf/x86_64/*.conf      $(TARGET_DIR)/etc/triggerhappy/triggers.d/
endef
