...
define BATOCERA_AUDIO_ALSA_INSTALL_TARGET_CMDS
	...
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio-alsa/alsa/asoundrc-* \
		$(TARGET_DIR)/usr/share/reglinux/alsa/
	# sample audio files
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio-alsa/*.wav $(TARGET_DIR)/usr/share/sounds
	# init script
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio-alsa/S01audio \
		$(TARGET_DIR)/etc/init.d/S01audio
	# udev script to unmute audio devices
	install -m 0644 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio-alsa/90-alsa-setup.rules \
		$(TARGET_DIR)/etc/udev/rules.d/90-alsa-setup.rules
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio-alsa/soundconfig \
		$(TARGET_DIR)/usr/bin/soundconfig
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio-alsa/alsa/batocera-audio$(ALSA_SUFFIX) \
