...
define BATOCERA_AUDIO_INSTALL_TARGET_CMDS
	...
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/alsa/asoundrc-* \
		$(TARGET_DIR)/usr/share/reglinux/alsa/

	# sample audio files
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/*.wav \
	    $(TARGET_DIR)/usr/share/sounds

	# init script
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/Saudio \
		$(TARGET_DIR)/etc/init.d/S06audio

	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/S27audioconfig \
		$(TARGET_DIR)/etc/init.d/S27audioconfig
	# udev script to unmute audio devices
	install -m 0644 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/90-alsa-setup.rules \
		$(TARGET_DIR)/etc/udev/rules.d/90-alsa-setup.rules
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/soundconfig \
		$(TARGET_DIR)/usr/bin/soundconfig
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/alsa/batocera-audio \
		$(TARGET_DIR)/usr/bin/batocera-audio
	install -m 0755 $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/alsa/batocera-mixer \
		$(TARGET_DIR)/usr/bin/batocera-mixer

	# pipewire-pulse policy
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/pulseaudio-system.conf \
		$(TARGET_DIR)/etc/dbus-1/system.d
	...
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/50-alsa-config.lua \
		$(TARGET_DIR)/usr/share/wireplumber/main.lua.d/50-alsa-config.lua

	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/pipewire.conf \
		$(TARGET_DIR)/usr/share/pipewire/pipewire.conf
endef

define BATOCERA_AUDIO_X86_INTEL_DSP
	mkdir -p $(TARGET_DIR)/etc/modprobe.d
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/intel-dsp.conf \
...
define BATOCERA_AUDIO_STEAM_DECK_OLED
	...
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/sof-vangogh-*.bin \
	    $(TARGET_DIR)/lib/firmware/amd/sof/
	mkdir -p $(TARGET_DIR)/lib/firmware/amd/sof-tplg
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/sof-vangogh-nau8821-max.tplg \
	    $(TARGET_DIR)/lib/firmware/amd/sof-tplg/sof-vangogh-nau8821-max.tplg
	# extra ucm files
	mkdir -p $(TARGET_DIR)/usr/share/alsa/ucm2
	cp -pr $(BR2_EXTERNAL_BATOCERA_PATH)/package/core/batocera-audio/ucm2/* \
