...
qemu-system-x86_64 -cpu host -enable-kvm -device intel-hda -device hda-duplex -vga virtio -device virtio-gpu-pci -smp 2 -m 2048 -device e1000,netdev=net0 -netdev user,id=net0,hostfwd=tcp::5555-:22 -device nec-usb-xhci "${JOYSTICK_CMD}" -drive "format=raw,file=${REGLINUX_IMG}" "${SHARE_CMD}"
