...
MAME_VERSION=$(cat package/reglinux/emulators/mame/mame.mk | grep GroovyMAME | grep Version | cut -d " " -f 4)
LRMAME_VERSION=$(cat package/reglinux/emulators/libretro/libretro-mame/libretro-mame.mk | grep LIBRETRO_MAME_VERSION | grep lrmame | cut -d " " -f 3 | sed s/lrmame//g | sed s/0/0\./)
