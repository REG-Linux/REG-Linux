...
BUILD_DIR="${REGLINUX_BINARIES_DIR}/build-uboot-gtking"
mkdir -p "${BUILD_DIR}" || exit 1
cp "${BUILD_SCRIPT}" "${BUILD_DIR}/" || exit 1
cd "${BUILD_DIR}" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${REGLINUX_BINARIES_DIR}" "beelink-gtking_defconfig" "beelink-s922x" "gtking" || exit 1
