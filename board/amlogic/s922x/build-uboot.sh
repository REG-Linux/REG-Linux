...
Downloads a cached U-Boot 2024.01, applies the shared patches, and runs
LibreELEC's amlogic-boot-fip helper inside IMAGES_DIR/uboot-<target>.
EOF
}
...
readonly UBOOT_VERSION=2024.01
...
apply_patch_dir "${BR2_EXTERNAL_REGLINUX_PATH}/board/amlogic/s922x/patches/uboot"
echo "Shared patches applied"
apply_patch_dir "${BOARD_DIR}/patches/uboot"
echo "Board patches applied"
...
RESULT_DIR="${IMAGES_DIR}/uboot-${UBOOT_TARGET}"
mkdir -p "${RESULT_DIR}"
