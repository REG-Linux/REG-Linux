...
BUILD_DIR="${REGLINUX_BINARIES_DIR}/build-uboot-vim3"
mkdir -p "${BUILD_DIR}" || exit 1
cp "${BUILD_SCRIPT}" "${BUILD_DIR}/" || exit 1
cd "${BUILD_DIR}" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${REGLINUX_BINARIES_DIR}" "khadas-vim3_defconfig" "khadas-vim3" "vim3" || exit 1
