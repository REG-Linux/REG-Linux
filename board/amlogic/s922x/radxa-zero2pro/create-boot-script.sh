...
BUILD_DIR="${REGLINUX_BINARIES_DIR}/build-uboot-zero2"
mkdir -p "${BUILD_DIR}" || exit 1
cp "${BUILD_SCRIPT}" "${BUILD_DIR}/" || exit 1
cd "${BUILD_DIR}" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${REGLINUX_BINARIES_DIR}" "radxa-zero2_defconfig" "radxa-zero2" "zero2" || exit 1
