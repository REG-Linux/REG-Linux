...
BUILD_DIR="${REGLINUX_BINARIES_DIR}/build-uboot-odroidc2"
mkdir -p "${BUILD_DIR}" || exit 1
cp "${BUILD_SCRIPT}" "${BUILD_DIR}/" || exit 1
cd "${BUILD_DIR}" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${REGLINUX_BINARIES_DIR}" "odroid-c2_defconfig" "odroid-c2" "odroidc2" || exit 1
