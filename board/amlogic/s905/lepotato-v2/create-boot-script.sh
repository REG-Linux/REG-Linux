...
BUILD_DIR="${REGLINUX_BINARIES_DIR}/build-uboot-lepotato"
mkdir -p "${BUILD_DIR}" || exit 1
cp "${BUILD_SCRIPT}" "${BUILD_DIR}/" || exit 1
cd "${BUILD_DIR}" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${REGLINUX_BINARIES_DIR}" "libretech-cc_v2_defconfig" "lepotato" "lepotato" || exit 1
