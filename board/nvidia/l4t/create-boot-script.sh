...
# Copy all Tegra T210 device tree blobs
mkdir -p "${REGLINUX_BINARIES_DIR}/boot/boot/dtb/nvidia" || exit 1
for dtb in "${BINARIES_DIR}"/tegra210-*.dtb; do
    [ -f "${dtb}" ] && cp "${dtb}" "${REGLINUX_BINARIES_DIR}/boot/boot/dtb/nvidia/" || true
done
