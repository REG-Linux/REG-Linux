...
| `khadas-edge-2` | yes | `/boot/rk3588s-khadas-edge2.dtb` | standard pipeline. |
...
1. To add a new board, copy one of the existing board directories and adjust `boot/extlinux.conf`, `genimage.cfg`, and optionally the DTB name. Add a `build-uboot.sh` if the board needs a custom U-Boot or you want to reuse the generic Rockchip build.
