...
BUILD_DIR="${REGLINUX_BINARIES_DIR}/build-uboot-rock-3a"
mkdir -p "${BUILD_DIR}" || exit 1
cp "${BUILD_SCRIPT}" "${BUILD_DIR}/" || exit 1
cd "${BUILD_DIR}" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${REGLINUX_BINARIES_DIR}" "rock-3a-rk3568_defconfig" "rock-3a" || exit 1
...
UBOOT_DIR="$(dirname "${REGLINUX_BINARIES_DIR}")/uboot-rock-3a"
