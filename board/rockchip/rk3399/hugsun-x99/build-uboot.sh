...
# Copy output
cp "${IMAGES_DIR}/reglinux/build-uboot-hugsun-x99/u-boot-${UBOOT_VERSION}/idbloader.img" "${IMAGES_DIR}/reglinux/uboot-hugsun-x99/idbloader.img"

# Rockchip trust.img
cd ../rkbin
./tools/trust_merger RKTRUST/RK3399TRUST.ini
# Copy output
cp "${IMAGES_DIR}/reglinux/build-uboot-hugsun-x99/rkbin/trust.img" "${IMAGES_DIR}/reglinux/uboot-hugsun-x99/trust.img"

# Pack U-Boot binary with Rockchip loaderimage tool
cd ../u-boot-${UBOOT_VERSION}
../rkbin/tools/loaderimage --pack --uboot u-boot-dtb.bin uboot.img 0x200000
# Copy output
cp "${IMAGES_DIR}/reglinux/build-uboot-hugsun-x99/u-boot-${UBOOT_VERSION}/uboot.img" "${IMAGES_DIR}/reglinux/uboot-hugsun-x99/uboot.img"
