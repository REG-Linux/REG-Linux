...
Builds U-Boot 2025.01 for RK3399 boards during the post-image step. The helper
...
idbloader.img/u-boot.itb under ${IMAGES_DIR}/uboot-${UBOOT_TARGET}/ for genimage.
EOF
}
...
declare -a MAKE_ENV
MAKE_ENV+=("BL31=${RKBIN_CACHE}/bin/rk33/rk3399_bl31_v1.36.elf")
MAKE_ENV+=("ROCKCHIP_TPL=${RKBIN_CACHE}/bin/rk33/rk3399_ddr_800MHz_v1.30.bin")
MAKE_ENV+=("CROSS_COMPILE=${TOOLCHAIN_PREFIX}")

NPROC=$(nproc 2>/dev/null || echo 1)

pushd "${UBOOT_SRC}" >/dev/null

env "${MAKE_ENV[@]}" ARCH=aarch64 make O="${UBOOT_BUILD_DIR}" "${UBOOT_DEFCONFIG}"
...
if [ "${#CONFIG_FRAGMENTS[@]}" -gt 0 ]; then
    ...
    env "${MAKE_ENV[@]}" ARCH=aarch64 make O="${UBOOT_BUILD_DIR}" olddefconfig
fi

env "${MAKE_ENV[@]}" ARCH=aarch64 make -j"${NPROC}" O="${UBOOT_BUILD_DIR}"

popd >/dev/null

for required in idbloader.img u-boot.itb; do
    if [ ! -f "${UBOOT_BUILD_DIR}/${required}" ]; then
        echo "ERROR: ${UBOOT_BUILD_DIR}/${required} missing after build" >&2
        exit 1
    fi
done
...
cp "${UBOOT_BUILD_DIR}/u-boot.itb" "${DEST_DIR}/u-boot.itb"
[ -f "${UBOOT_BUILD_DIR}/u-boot-rockchip.bin" ] && cp "${UBOOT_BUILD_DIR}/u-boot-rockchip.bin" "${DEST_DIR}/u-boot-rockchip.bin"
