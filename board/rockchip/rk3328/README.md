...
- `renegade/` – Renegade RK3328 board layout. The `boot/extlinux.conf`
  boots with `/boot/rk3328-roc-cc.dtb`. `create-boot-script.sh` now builds
  U-Boot locally during the post-image stage (see `../build-uboot.sh`) and
  `genimage.cfg` consumes the staged `../uboot-roc-cc-rk3328/u-boot-rockchip.bin`
  instead of the old `uboot-multiboard` bundle. The helper rebuilds TF-A (BL31)
  from source and still pulls the DDR TPL from `rkbin`, and it also folds in the
  QoS tweak that used to live in `common-rk3328/0004-*` so HDMI output stays
  glitch-free under load.
- `rock64/` – ROCK64-specific variant. It mirrors the Renegade structure but
  uses `rk3328-rock64.dtb` and the `../uboot-rock64-rk3328/u-boot-rockchip.bin`
  payload generated by the local U-Boot helper, so the correct DTB and bootloader
  land in the VFAT partition.
