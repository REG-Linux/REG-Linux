...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-orangepi-zero2w"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-orangepi-zero2w/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-orangepi-zero2w/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/sun50i-h618-orangepi-zero2w.dtb"    "${REGLINUX_BINARIES_DIR}/boot/boot/"       || exit 1
