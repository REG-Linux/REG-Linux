...
# xu20-v32: U-Boot loads uImage + uInitrd from FAT partition
cp "${BOARD_DIR}/uImage"                "${REGLINUX_BINARIES_DIR}/boot/uImage"                  || exit 1
cp "${BOARD_DIR}/uInitrd"               "${REGLINUX_BINARIES_DIR}/boot/uInitrd"                 || exit 1
...
# xu20-v32: boot.img has no ramdisk (both slots are kernel images).
# The initramfs is loaded from uInitrd on the FAT partition instead.
# Patch uInitrd to inject our /init script.
bash "${BOARD_DIR}/../common/patch-uinitrd.sh" \
    "${REGLINUX_BINARIES_DIR}/boot/uInitrd" \
    "${BOARD_DIR}/../common/init"                                                               || exit 1
