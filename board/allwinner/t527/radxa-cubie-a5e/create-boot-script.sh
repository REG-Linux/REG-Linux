...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-radxa-cubie-a5e"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-radxa-cubie-a5e/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-radxa-cubie-a5e/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/sun55i-a527-radxa-a5e.dtb"   "${REGLINUX_BINARIES_DIR}/boot/boot/"       || exit 1
cp "${BINARIES_DIR}/sun55i-t527-orangepi-4a.dtb" "${REGLINUX_BINARIES_DIR}/boot/boot/"       || exit 1
