...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-radxa-zero"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-radxa-zero/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-radxa-zero/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-g12a-radxa-zero.dtb"  "${REGLINUX_BINARIES_DIR}/boot/boot/"     || exit 1
