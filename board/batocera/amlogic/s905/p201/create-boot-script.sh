...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-p201"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-p201/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-p201/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-gxbb-nexbox-a95x.dtb" "${REGLINUX_BINARIES_DIR}/boot/boot/"     || exit 1
