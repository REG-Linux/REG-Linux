...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-odroidc4"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"  "${REGLINUX_BINARIES_DIR}/build-uboot-odroidc4/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-odroidc4/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-sm1-odroid-c4.dtb" "${REGLINUX_BINARIES_DIR}/boot/boot/"        || exit 1
