...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-gtking"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-gtking/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-gtking/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-g12b-gtking.dtb" "${REGLINUX_BINARIES_DIR}/boot/boot/"     || exit 1
