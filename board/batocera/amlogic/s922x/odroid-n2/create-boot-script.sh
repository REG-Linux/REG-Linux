...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-odroidn2"                          || exit 1
cp "${BOARD_DIR}/build-uboot.sh" "${REGLINUX_BINARIES_DIR}/build-uboot-odroidn2/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-odroidn2/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-g12b-odroid-n2.dtb"      "${REGLINUX_BINARIES_DIR}/boot/boot/meson-g12b-odroid-n2.dtb" || exit 1
