...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-bananapi-m4-berry"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-bananapi-m4-berry/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-bananapi-m4-berry/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/sun50i-h618-bananapi-m4berry.dtb" "${REGLINUX_BINARIES_DIR}/boot/boot/"       || exit 1
