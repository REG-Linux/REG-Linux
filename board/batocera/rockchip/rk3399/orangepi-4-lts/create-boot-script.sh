...
mkdir -p "${REGLINUX_BINARIES_DIR}/build-uboot-orangepi-4-lts"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${REGLINUX_BINARIES_DIR}/build-uboot-orangepi-4-lts/" || exit 1
cd "${REGLINUX_BINARIES_DIR}/build-uboot-orangepi-4-lts/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/rk3399-orangepi-4-lts.dtb"  "${REGLINUX_BINARIES_DIR}/boot/boot/"       || exit 1
