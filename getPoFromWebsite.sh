#!/bin/bash

if ! wget -q "http://translations.batocera.org/?q=updatable&type=options" -O - |
	while read L
	do
	    echo -n "updating ${L}..." >&2

	    if ! test -d "package/emulationstation/es-system/locales/${L}"
	    then
		echo "directory package/emulationstation/es-system/locales/${L} doesn't exist" >&2
		exit 1
	    fi

	    TARGET="package/emulationstation/es-system/locales/${L}/es-system.po"
	    if ! wget -q "http://translations.batocera.org/po/options_${L}.po" -O "${TARGET}"
	    then
		echo "unable to find file on translations.batocera.org" >&2
		exit 1
	    fi
	    printf "\t"
	    dos2unix "${TARGET}" 2>/dev/null
	    msgfmt --statistics "${TARGET}"

	done
then
    echo "failed" >&2
    exit 1
fi

exit 0
